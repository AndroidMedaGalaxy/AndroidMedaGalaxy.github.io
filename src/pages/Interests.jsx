            ...
            {images.length > 1 && (
                <div className="flex items-center justify-center gap-4 py-2">
                    <button
                        ...
                        className="bg-slate-800/60 dark:bg-black/60 text-white font-bold text-2xl rounded-full w-10 h-10 flex items-center justify-center hover:bg-slate-800/80 dark:hover:bg-black/80 shadow-md"
                        aria-label="Previous"
                    >
                        &#8592;
                    </button>
                    <div className="flex gap-2">
                        {images.map((_, i) => (
                            <button
                                key={i}
                                className={`inline-block w-3 h-3 rounded-full border border-slate-400 dark:border-slate-600 ${i === idx ? 'bg-teal-500/70 dark:bg-teal-300/70' : 'bg-slate-400/60 dark:bg-slate-500/60'}`}
                                onClick={() => setIdx(i)}
...
function CardImage({images, title}) {
    ...
    if (imgError || !mainImg) {
        return (
            <div
                className="h-40 w-full flex items-center justify-center rounded-t-xl border-b border-slate-300 dark:border-slate-800 bg-gradient-to-br from-teal-400 via-slate-300 to-slate-100 dark:from-teal-700 dark:via-slate-800 dark:to-slate-950">
                <span className="text-lg font-bold text-slate-900 dark:text-slate-100 drop-shadow text-center px-4">{title}</span>
            </div>
        );
    }
    return (
...
export default function Interests() {
    return (
        <main className="mx-auto max-w-5xl px-4 pb-12 pt-8">
            <h1 className="text-3xl font-bold text-slate-900 dark:text-slate-50">Interests</h1>
            ...
            <div className="mt-8 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                {interests.map(interest => (
                    <div key={interest.name}
                         className="flex flex-col rounded-xl border border-slate-300 dark:border-slate-800 bg-white dark:bg-slate-900/70 shadow-lg hover:scale-[1.025] transition-transform">
                        ...
                        <div className="flex-1 p-4 flex flex-col">
                            <h2 className="text-lg font-bold text-teal-600 dark:text-teal-300 mb-1">{interest.name}</h2>
                            <p className="text-sm text-slate-700 dark:text-slate-200 mb-3">{interest.description}</p>
                            <div className="flex flex-wrap gap-2 mb-3">
                                {interest.tags.map(tag => (
                                    <span key={tag}
                                          className="rounded-full border border-slate-300 dark:border-slate-800 bg-slate-100 dark:bg-slate-900 px-2 py-0.5 text-[11px] text-slate-700 dark:text-slate-200">{tag}</span>
                                ))}
                            </div>
                        </div>
                    </div>
